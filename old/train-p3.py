...
def load_emotion_data_for(character):
    ...
    print("(training) loaded {} images & {} labels for {}...".format(len(emotions_training), len(emotion_training_labels),
                                                                    character))
    print("(testing) loaded {} images & {} labels for {}...".format(len(emotions_testing), len(emotion_testing_labels),
                                                                    character))
    ...
    if visualise is True:
        ...
        for e, i in enumerate(range(n)):
            ...
            if ground_truth_emotion == predicted_emotion:
                ...
                print("image prediction: {} | confidence score: ({:.1f}%)".format(COLOR['G'] + predicted_emotion + COLOR['RS'],
                                                                                confidence_score))
            else:
                plt.text(3, 7, predicted_emotion.title(), fontsize=36, color="red")
                print("image prediction: {} | confidence score: ({:.1f}%)".format(COLOR['R'] + predicted_emotion + COLOR['RS'],
                                                                                confidence_score))
...
def classify_emotion_from_image(local_image):
    ...
    if os.path.isfile('model_{}_.h5'.format(model_version)):
        ...
        confidence_score = float(prob[pred_class] * 100)
        print("image: {}\n---".format(sys.argv[2]))
        print("image prediction: {} | confidence score: ({:.1f}%)".format(
            COLOR['G'] + predicted_emotion + COLOR['RS'], confidence_score))
