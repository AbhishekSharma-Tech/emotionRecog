...
def detect(character, video, show_video=True):
    ...
    results_path = os.path.join('results/' + "tom_jerry")
    
    # make a folder in results for our recognized faces.
    ...
    while(1):
        ...
        for (x, y, w, h) in faces:
            ...
            if character['save'] == True:
                cv2.imwrite(results_path + '/' + character['name'] + '_frame_' + frame_number + '.png', roi)
...
def process(character):
    ...
    for video in enumerate(videos):
        episode = video[1].split('- ')[0].split('(')[0].strip()
